  ...
  methods: {
    loadPacks() {
      this.file.fs.readdir(this.steamPath, (error, files) => {
        this.panels = files
          .filter((x) => x.startsWith("The Jackbox Party Pack"))
    ...
    toggle(index) {
      ...
      this.showTip = this.panels.any(x=>x)
      this.fact = ToolFacts.getFact();
      this.factText = ToolFacts.getPaginationText();
    },
    set(index, state) {
    ...
    itemClick(item, index) {
      if (item.id === 0) {
        ...
        if (this.items[index].isMod) {
          ...
          this.file.openInFileExplorer(
            [
              this.steamPath,
              "The Jackbox Party Pack" +
                (this.items[index].id > 1 ? " " + this.items[index].id : ""),
            ].join("\\")
          );
        }
      } else if (item.id === 2) {
            ...
            this.file
              ...
              .then((success) => {
                if (success) {
                  ...
                  window.file.fs.writeFileSync(
                    [this.modPath, this.answer, "modSettings.json"].join("\\"),
                    JSON.stringify(json)
                  );
