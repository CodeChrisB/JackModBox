...
div(style="overflow: auto;max-height:calc(100vh - 26px);max-width:80vw").view.ma-4.mr-6.mb-6
  div(v-for="(line,index) in markdown")
    span.pl-3(v-if="line.startsWith('#')" :class="genHeaderClass(line)") {{ replaceLeadingHashes(line) }}
    span.pl-3(v-else-if="line.startsWith('~~ ')").font-italic {{ line.slice(2) }}
    div.pl-3(v-else-if="regexLine(line,'img',false)")
      v-img(
    ...
    div(v-else-if="line.startsWith('---')")
      v-divider
    div.pl-3(v-else-if="isEmpty(line)")
      div(style="min-height:24px")
    div.pl-3(v-else-if="line.startsWith('||')")
      div(style="min-height:12px")
    div.pl-3(v-else-if="regexLine(line,'icon',false)")
      v-icon(
    ...
    div.pl-3(v-else-if="regexLine(line,'bicon',false)")
      span {{ regexLine(line,'bicon',true).value }}
      v-icon(
        :color="regexLine(line,'bicon',true).attribute.split(' ')[1]"
        ) {{ regexLine(line,'bicon',true).attribute.split(' ')[0] }}

    div.pl-3(v-else-if="regexLine(line,'a',false)")
...
export default {
  ...
  methods: {
    ...
    getImageDimension(str, dim) {
      ...
      return dimension[1].split('x')[dim === 'w' ? 0 : 1]
    },
    isEmpty(line) {
      return line === '\r'
    },
